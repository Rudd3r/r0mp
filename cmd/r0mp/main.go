		...
		Commands: []args.Command{
			...
			&args.ParentCommand{
				...
				SubCommands: []args.Command{
					...
					&args.Cmd[domain.CommandSetupClean]{
						Names:            []string{"clean", "reset"},
						Description:      "Delete all r0mp state (cache, config, and data directories)",
						ShortDescription: "Delete all state",
						Flags: func(cfg *domain.CommandSetupClean, flags *flag.FlagSet) {
							flags.BoolVarP(
								&cfg.Force,
								"force", "f", false,
						...
						PositionalArgs: []*args.PositionalArg[domain.CommandSetupClean]{},
						Run: func(ctx context.Context, log *slog.Logger, cfg *domain.Config, cmdCfg *domain.CommandSetupClean) error {
							dirs := []struct {
								name string
								path string
							}{
							...
							if !cmdCfg.Force {
								fmt.Print("Are you sure you want to delete all r0mp state? This cannot be undone. [y/N]: ")
								var response string
							...
							for _, dir := range dirs {
								...
								if _, err := os.Stat(dir.path); os.IsNotExist(err) {
									fmt.Printf("⚠️  %s directory does not exist: %s\n", dir.name, dir.path)
									continue
								}
								fmt.Printf("Deleting %s directory: %s\n", dir.name, dir.path)
								if err := os.RemoveAll(dir.path); err != nil {
									fmt.Printf("❌ Failed to delete %s directory: %v\n", dir.name, err)
									errors = append(errors, fmt.Errorf("failed to delete %s directory: %w", dir.name, err))
								} else {
									fmt.Printf("✅ Deleted %s directory\n", dir.name)
								}
							}
							...
							fmt.Println("✅ All r0mp state has been deleted successfully!")
							return nil
						},
					},
				},
