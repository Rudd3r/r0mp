...
func (q *QEMU) Run() (err error) {
	...
	var serialCharDev string
	switch runtime.GOOS {
	case "darwin", "linux":
		serialCharDev = "stdio,signal=off,id=char0"
	case "windows":
		serialCharDev = "console,id=char0"
	default:
	...
	stderr, err := q.cmd.StderrPipe()
	if err != nil {
		return fmt.Errorf("could not get stdout pipe: %v", err)
	}

	q.eg.Go(q.cmd.Run)
	q.eg.Go(func() error { return q.tailLog(stdout, "stdout") })
	q.eg.Go(func() error { return q.tailLog(stderr, "stderr") })

	return q.eg.Wait()
}

func (q *QEMU) tailLog(f io.Reader, source string) (err error) {
