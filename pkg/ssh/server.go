	...
	for {
		...
		case req, ok := <-requests:
			...
			case "exec":
				...
				logOutput := false
				for _, e := range env {
					if strings.HasPrefix(e, domain.EnvRaftDetach+"=") {
						detach = strings.HasSuffix(e, "=1")
					}
					if strings.HasPrefix(e, domain.EnvRaftLogOutput+"=") {
...
func extractSessionConfig(env []string) (*sessionConfig, error) {
	...
	for _, e := range env {
		...
		switch parts[0] {
		case domain.EnvRaftChroot:
			cfg.Chroot = parts[1]
		case domain.EnvRaftUser:
...
func filterSessionEnv(env []string) []string {
	...
	for _, e := range env {
		if strings.HasPrefix(e, domain.EnvRaftChroot+"=") ||
			strings.HasPrefix(e, domain.EnvRaftUser+"=") ||
			strings.HasPrefix(e, domain.EnvRaftDetach+"=") ||
			strings.HasPrefix(e, domain.EnvRaftLogOutput+"=") {
			continue
		}
