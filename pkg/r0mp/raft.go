...
func (s *Raft) buildInit() error {
	...
	initBuilder, err := initbuilder.NewInitFS(assets.Initrd(), initFile)
	if err != nil {
	...
	if s.raftCfg.Image != "" {
		initBuilder.AddChroot(initbuilder.NewStandardChroot(s.raftCfg.Image, "/mnt/rootfs"))
	}
...
func (s *Raft) writeImage() error {
	...
	layerClient := client.NewLayerClient(s.sshClient, slog.Default())
