...
func (s *Cove) Exec(cmd *domain.CommandExec) error {
	...
	if rafts[0].Image != "" && !cmd.NoChroot {
		env[domain.EnvRaftChroot] = "/mnt/rootfs"
		env[domain.EnvRaftUser] = rafts[0].User
	}
	for k, v := range cmd.Environment {
		env[k] = v
	}
	...
	return client.Client(s.ctx, s.log, &domain.SSHClientConfig{
