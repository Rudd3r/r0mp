...
func TestQemuPath(t *testing.T) {
	...
	t.Run("adds exe extension on windows", func(t *testing.T) {
		...
		if err == nil {
			...
			switch runtime.GOARCH {
			case "amd64":
				assert.Contains(t, path, "qemu-system-x86_64.exe")
			case "arm64":
				assert.Contains(t, path, "qemu-system-aarch64.exe")
			}
		} else {
			// QEMU not installed in PATH or Program Files, which is fine for testing
			t.Logf("QEMU not found on Windows system (expected in CI): %v", err)
		}
	})
}
