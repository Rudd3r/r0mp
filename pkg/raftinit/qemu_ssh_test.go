...
import (
	...
	initbuilder "github.com/Rudd3r/r0mp/pkg/raftinit/builder"
	sshpkg "github.com/Rudd3r/r0mp/pkg/ssh"
	"github.com/Rudd3r/r0mp/pkg/ssh/client"
...
func TestQEMUIntegration_SSHPasswordAuth(t *testing.T) {
	...
	initPath := buildAndCreateInitrd(t, testDir, func(builder *initbuilder.InnitFSBuilder) error {
		// Configure network
	...
	err := client.Client(clientCtx, log, clientCfg)
	...
	defer clientCancel2()
	err = client.Client(clientCtx2, log, clientCfg)
	require.NoError(t, err, "SSH client should connect successfully")
...
func TestQEMUIntegration_SSHLayerWriter(t *testing.T) {
	...
	layerClient := client.NewLayerClient(sshClient, slog.Default())
...
func TestQEMUIntegration_SSHWithChroot(t *testing.T) {
	...
	initPath := buildAndCreateInitrd(t, testDir, func(builder *initbuilder.InnitFSBuilder) error {
		...
		chroot := initbuilder.NewStandardChroot("approot", "/mnt/approot")
...
func TestQEMUIntegration_SSHDetachExec(t *testing.T) {
	...
	t.Run("detach_long_running_foreground_process", func(t *testing.T) {
		...
		err = client.Client(checkCtx, log, checkCfg)
	...
	t.Run("detach_env_var_filtered", func(t *testing.T) {
		...
		checkCfg := &domain.SSHClientConfig{
			...
			Host:            "127.0.0.1",
			Port:            sshPort,
			Command:         fmt.Sprintf("grep %s /tmp/detach_env || echo NO_%s", domain.EnvRaftDetach, domain.EnvRaftDetach),
			Auth:            []ssh.AuthMethod{ssh.PublicKeys(signer)},
			HostKeyCallback: ssh.InsecureIgnoreHostKey(),
			Stdout:          &stdout,
			Stderr:          &bytes.Buffer{},
		...
		assert.Contains(t, stdout.String(), fmt.Sprintf("NO_%s", domain.EnvRaftDetach), "RAFT_DETACH should be filtered")
	})
